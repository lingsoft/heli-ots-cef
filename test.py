...
class TestResponseStucture(unittest.TestCase):
    ...
    def test_api_response_status_code(self):
        ...
        status_code = requests.post(self.base_url, headers=self.headers, data=self.payload).status_code
    ...
    def test_valid_api_response(self):
        ...
        response = requests.post(self.base_url, headers=self.headers, data=self.payload).json()['response']
    ...
    def test_api_response_content_offset(self):
        ...
        for result in fin_result:
            self.assertEqual(result['start'], self.texts.find(result['features']['original_text']))
            self.assertEqual(result['end'], result['start'] + len(result['features']['original_text']))
